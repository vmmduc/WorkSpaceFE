...
export class SignalRService {
  ...
  getConnection(): Promise<signalR.HubConnection> {
    var signalRConnection = this._connectionPromise.then(
      () => this._connection
    );
    return signalRConnection;
  }

  async SendMessageToConversationId(
    messageContent: SendMessageObj
  ): Promise<void> {
    if (this._connection.state === 'Connected') {
      var request: BaseRequest<SendMessageObj> = { Param: messageContent };
      await this._connection
        .invoke('SendMessageToConversationId', request)
        .catch((error) => {
  ...
  onListenReceive(evenName: string, callback: (...args: any[]) => void) {
    this._connection.on(evenName, (...args: any) => {
      callback(...args);
    });
  }
  ...
  async PreparingMessage(preparingMessage: PreparingMessageObj): Promise<void> {
    if (this._connection.state === 'Connected') {
      var request: BaseRequest<PreparingMessageObj> = {
        Param: preparingMessage,
      };
      await this._connection
        .invoke('PreparingMessage', request)
        .catch((error) => {
          console.error(`Error sending message to user: ${error}`);
        });
    } else {
  ...
  async setUserOnline(): Promise<void> {
    ...
    if (!token) {
      ...
      this._connection
        .invoke('SetUserOnline', this.dataLoca.getId())
        .catch((error) => {
